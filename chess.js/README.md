...
Clears the board and loads the provided FEN string. The castling rights, en passant square and move numbers are defaulted to - - 0 1 if omitted. Throws an exception if the FEN is invalid.
...
.isCapture() - is the move a regular capture? NOTE: this is false for an en-passant capture
...
Remove a field from the PGN header. Returns true if the header was removed, else false as the header was not found.
...
Sets the castling rights for the given color. Returns true if the change was successfully made. False will be returned when the position doesn't allow the requested change i.e. if the corresponding king or rook is not on it's starting square.
...
chess.js/README.md at master · jhlywa/chess.js 